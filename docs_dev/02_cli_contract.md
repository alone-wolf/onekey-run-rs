...
- `onekey-run up`
  ...
  若配置了顶层 `log`，实例生命周期事件以及 hook/action 的执行状态会额外落盘到实例日志。
...
- `onekey-run up -d` / `onekey-run up --daemon`
  ...
  若配置了顶层 `log`，应由后台 supervisor 负责持续写入实例日志。
- `onekey-run up --tui`
...
- `onekey-run check`
  仅校验配置、依赖图和命令可执行性。
  当配置包含顶层 `log`、`actions`、`hooks` 时，也应校验日志组合合法性、路径冲突以及 hook/action 引用关系。
...
- `--tui`
  ...
  顶层实例 `log` 若已配置，TUI 期间产生的 hook/action 生命周期状态也应写入实例日志。
- `-d, --daemon`
  后台运行模式。
